...
"#,
  ...
  let events: Vec<serde_json::Value> = String::from_utf8(output.stdout)
    ...
    .collect();
  assert!(events.iter().all(|event| event["schema_version"] == 23));
...
fn runtime_inventory_json_uses_the_shared_event_batch() {
  ...
  assert!(output.status.success(), "{}", String::from_utf8_lossy(&output.stderr));
  let stdout = String::from_utf8(output.stdout).unwrap();
  assert!(stdout.contains("\"schema_version\":23"), "{stdout}");
...
fn project_root_discovers_git_without_project_selection() {
  ...
  let event = String::from_utf8(json.stdout)
    ...
    .unwrap();
  assert_eq!(event["schema_version"], 23);
...
#[test]
fn project_inputs_report_distinct_ancestor_precedence_without_parsing_files() {
  let temp = TempDirectory::new();
  temp.write("global.json", "{ malformed but discovery does not parse");
  temp.write("Directory.Build.props", "not XML");
  temp.write("Directory.Build.targets", "root target");
  temp.write("NuGet.Config", "root config");
  temp.write("child/Directory.Build.targets", "child target");
  temp.write("child/Directory.Packages.props", "child packages");
  temp.write("child/NuGet.Config", "child config");
  let project = temp.write("child/src/App.csproj", "not XML");

  let output = dv().args(["--json", "project", "inputs"]).arg(&project).output().unwrap();
  ...
  let event = String::from_utf8(output.stdout)
    ...
    .lines()
    .map(|line| serde_json::from_str::<serde_json::Value>(line).unwrap())
    .find(|event| event["type"] == "workspace_inputs_discovered")
    .unwrap();
  assert_eq!(event["schema_version"], 23);
  assert_eq!(event["global_json"], temp.0.join("global.json").to_string_lossy().as_ref());
  assert_eq!(event["directory_build_props"], temp.0.join("Directory.Build.props").to_string_lossy().as_ref());
  assert_eq!(
    event["directory_build_targets"],
    temp.0.join("child").join("Directory.Build.targets").to_string_lossy().as_ref()
  );
  assert_eq!(
    event["directory_packages_props"],
    temp.0.join("child").join("Directory.Packages.props").to_string_lossy().as_ref()
  );
  let configs = event["nuget_configs"].as_array().unwrap();
  assert_eq!(configs[configs.len() - 2], temp.0.join("NuGet.Config").to_string_lossy().as_ref());
...
#[test]
fn project_inputs_validate_operands_before_filesystem_io() {
  let temp = TempDirectory::new();
  let output = dv().args(["project", "inputs", "missing", "second"]).current_dir(&temp.0).output().unwrap();

  assert_eq!(output.status.code(), Some(2));
  let stderr = String::from_utf8(output.stderr).unwrap();
  assert!(stderr.contains("error[DV0002]"), "{stderr}");
  assert!(stderr.contains("at most one path"), "{stderr}");
  assert!(!stderr.contains("search start"), "{stderr}");
}
