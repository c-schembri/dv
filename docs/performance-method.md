...
| `cli_compat_manifest` | embedded manifest; byte-identity and schema/completeness preflight outside timing | `dv` process launch and one 270,090-byte static manifest write; Microsoft has no equivalent query and reports TBI |
...
| `cli_mode_classification` | immutable `small-console` fixture; tree snapshot and compatibility-profile rejection parity preflight outside timing | process launch, one-pass explicit mode classification, `dotnet`-profile exit selection, pre-I/O rejection, and output validation |
