...
| `cli_version` | installed SDK inventory; exact selected-version equality preflight outside timing | process launch, SDK discovery, selection, and compatible version output |
| `cli_protocol_version` | immutable `small-console` directory; both global-option placements for `self-version` validated outside timing | `dv` process launch and one validated schema-23 executable/protocol-version event batch; Microsoft has no equivalent command and reports TBI |
...
| `path_identity` | immutable paired MSBuild/SDK-project fixture; exact lexical missing-reference spelling, typed missing-project diagnostic, failed outcome, and zero-mutation preflight outside timing | process launch, root evaluation, one reference metadata query, lexical identity handling, structured diagnostic formatting, and failed status propagation |
| `source_link_traversal` | immutable three-source fixture plus a setup-time in-root directory link; exact logical Compile/source equality, typed source and project-reference cycle/escape controls, and zero-mutation preflight outside timing | process launch, project parse, recursive source scan, cold-branch link metadata and physical identity checks, sorted source materialization, and JSON output |
| `source_exclusions` | immutable two-source fixture plus setup-time `bin`, `obj`, and `.git` controls; exact Microsoft/`dv` source equality and zero-mutation preflight outside timing | process launch, project parse, fixed/configured exclusion construction, pruned recursive source scan, sorted source materialization, and JSON output |
| `filesystem_case_identity` | immutable root/library fixture; setup adds a lowercase library only when the active directory treats it as distinct, Microsoft assets and `dv` events must retain the resulting physical graph, and cleared sources prove network-free input | process launch, two-project cold restore on insensitive filesystems or three-project restore on sensitive filesystems, config spelling probes, shared offset path-table identity merge, lock/assets writes, and structured output |
...
cargo bench-all --case source_exclusions --samples 30 --warmups 5
```
