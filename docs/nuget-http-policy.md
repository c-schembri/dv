...
Output is a 16-byte, four-byte-aligned `PackageHttpPolicy`, one configured
reqwest client, and, only below the global 24-request ceiling, one shared Tokio
...
The cold authentication/source context is 104 bytes and eight-byte aligned; it
...
- TLS peer and hostname validation remains enabled. HTTP sources are rejected
  until the explicit opt-in contract in `NUGET-012` lands.
- General clients follow at most ten redirects and reject every non-HTTPS
  target. Client-certificate clients continue to reject all redirects.
...
text. Event schema 11 exposes only redacted policy values.
...
sources add one `Arc<Semaphore>`. A request holds one permit and one response
