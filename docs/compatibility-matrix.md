...
| NuGet storage and restore policy | global packages, HTTP cache, scratch, fallback folders, signature/audit modes, and proxy | Microsoft-compatible precedence, fallback lookup, scratch staging, atomic publication, authenticated proxy construction, bypass lists, bounded retry/timeout/rate-limit policy, secure redirects, offline suppression, and source-scoped HTTP/TLS opt-ins are implemented; unimplemented HTTP reuse, signature verification, and advisory execution fail or remain explicitly tracked |
| NuGet CLI restore overrides | repeatable configured HTTP, HTTPS, or local source, one explicit config, and one packages folder | Source replacement, config isolation, working-directory path normalization, configured security-policy inheritance, and config/environment precedence implemented for `restore` and `sync` |
...
| NuGet source credentials | merged `packageSourceCredentials`, exact per-source environment value, or self-contained cross-platform V2 provider; Basic/PAT over HTTPS or explicitly opted-in HTTP | NuGet-compatible static precedence, Windows DPAPI passwords, bounded provider handshake/claims/authentication, noninteractive CI, opt-in interactive output, cancellation/timeouts, zeroized plaintext, scheme/host/port containment, and redacted reporting implemented; DLL-only providers are rejected without a `dotnet` fallback |
| NuGet client certificates | merged `fileCert` PFX records or Windows store records per HTTPS source | Relative/absolute bounded PFX loading, DPAPI/clear passwords, `CurrentUser`/`LocalMachine` store names, thumbprint/private-key selection, source-origin containment, and redacted reporting implemented; other store selectors and non-Windows stores reject explicitly |
