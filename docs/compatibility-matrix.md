...
| NuGet source credentials | merged `packageSourceCredentials` or exact per-source environment value, Basic/PAT over HTTPS | NuGet-compatible precedence, Windows DPAPI passwords, zeroized plaintext, same-origin header containment, and redacted reporting implemented; credential providers remain planned |
